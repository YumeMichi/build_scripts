#!/usr/bin/env bash
# LineageOS build helper script, credit to @AOSPA
...
while true; do
    case "$1" in
        ...
        -b|--build-id|b|build-id) LINEAGE_BUILD_ID="$2"; shift;;
        -t|--build-type|t|build-type) LINEAGE_BUILD_VARIANT="$2"; shift;;
...
# Build signed rom package if specified
    ...
    sign_target_files_apks -o -d $KEY_MAPPINGS \
        "$OUT"/obj/PACKAGING/target_files_intermediates/lineage_$DEVICE-target_files-eng.$USER.zip \
...
# Build rom package
    ...
    mv -f $OUT/lineage_$DEVICE-ota-eng.$USER.zip $LINEAGE_VERSION.zip
